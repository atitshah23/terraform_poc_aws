...
resource "aws_instance" "phpapp" {
  ...
  echo "\$conn = new mysqli('localhost', 'root', 'password', 'test');" >> /var/www/html/calldb.php
...
resource "aws_instance" "database" {
  ...
  mysql -u root -psecret -e "create user 'root'@'%' identified by 'password';" mysql
  mysql -u root -psecret -e 'CREATE TABLE mytable (mycol varchar(255));' test
  mysql -u root -psecret -e "INSERT INTO mytable (mycol) values ('iamthebest') ;" test
HEREDOC
}
